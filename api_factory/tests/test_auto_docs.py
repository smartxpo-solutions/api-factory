...
from ..fields import StringField, IntegerField, BooleanField, ListField
...
class TestAutoDocs(TestCase):
    ...
    def test_simple_docs(self):
        ...
        expected_docs = {
            'description': 'Send a event review message to the event users',
            'http_method': 'POST',
            'input': {
                ...
                'event_id': {'description': 'Event ID',
                             ...
                             'required': True}},
            'output': {
                'is_sent': {'description': 'Was the letter sent?',
